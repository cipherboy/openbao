...
func TestTLSListener_SelfHostedNonStandard(t *testing.T) {
	...
	validateTLS(t, ctx, root, cluster.ClusterNodes[node].ContainerNetworkName, cluster.ClusterNodes[node].ContainerIPAddress+":8400")
}
...
func TestTLSListener_SelfHostedPrivileged(t *testing.T) {
	...
	// Validate ACME cert acquisition works.
	validateTLS(t, ctx, root, cluster.ClusterNodes[node].ContainerNetworkName, cluster.ClusterNodes[node].ContainerIPAddress+":443")
}
...
func validateTLS(t *testing.T, ctx context.Context, root string, networkName string, address string) {
